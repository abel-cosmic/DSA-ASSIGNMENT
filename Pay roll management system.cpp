...
//write Binary search algorithm here
	switch( searchUsing){
		case 'I'://using ID 	
		{
			...
			cin>>key;
			int low = 0, high = numberOfEmployees - 1, mid, found = 0, index = 1;
			do{
				mid = (low+high)/2;
					if(key>Data[mid].ID)
						low = mid + 1;
...
void Sort(){
	...
	if ( algo == 1 && arr == 1) {
			ascending_selection( RawData, use);
	}
	else if ( algo == 1 && arr == 2) {
		descending_selection(RawData, use);
	}
	
	else if ( algo ==2 && arr ==1) {
		ascending_bubble( RawData, use);
	}		
